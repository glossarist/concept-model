...
| Hierarchical — partitive | ISO 25964 / ISO 19135 | `broader_partitive`, `narrower_partitive`, `has_part`, `is_part_of` (binary); `PartitiveHyperedge` (n-ary, see <<partitive-hyperedges>>) |
...
introduces `PartitiveHyperedge` for *one-to-many* partitive decompositions where
the membership and diagram-notation metadata matter as a single
invariant. Each hyperedge carries:

- `comprehensive` — the whole concept being decomposed
- `parts` — one or more part concepts
- `enumeration` — `closed` (default; the encoded parts are ALL parts)
  or `open` (the encoded parts are SOME parts; others may exist)
- `markers` — diagram notation flags (`double`, `dashed`) from VIM
  concept-relationship diagrams
- `content` — optional localized label or note
...
See link:docs/design/partitive-hyperedge.md[] for the design rationale,
and `schemas/v3/examples/20-partitive-hyperedge-closed.yaml`,
`21-partitive-hyperedge-open.yaml`, `22-partitive-hyperedge-marked.yaml`,
and `23-partitive-hyperedge-plain.yaml` for example payloads.

Binary `broader_partitive` edges and `PartitiveHyperedge` coexist;
no automatic consolidation is performed.
