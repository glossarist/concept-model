...
| Hierarchical — partitive | ISO 25964 / ISO 19135 | `broader_partitive`, `narrower_partitive`, `has_part`, `is_part_of` (binary); `PartitiveRelation` (n-ary, see <<partitive-relations>>) |
...
In addition to the pairwise binary `broader_partitive` and
`narrower_partitive` relations above, glossarist's `[Unreleased]` work
introduces `PartitiveRelation` for *one-to-many* partitive decompositions
(ISO 704 rake notation) where the membership, completeness, and type-shared
plurality matter as a single invariant. Each relation carries:

- `comprehensive` — the superordinate concept partitive (the whole)
- `partitives` — two or more subordinate concepts partitive (the parts),
  each a `PartitiveMember` carrying a `ref` (ConceptRef) and optional
  `certainty` (confirmed/possible)
- `completeness` — `complete` (default; the encoded partitives fitted
  together constitute the comprehensive) or `partial` (other partitives
  exist beyond those encoded)
- `plurality` — optional type-shared plurality block encoding ISO 704's
  close-set double line (`is_shared`) and broken line (`is_uncertain`),
  plus optional `shared_type` (Glossarist extension)
- `criterion` — optional multilingual subdivision criterion (ISO 12620
  coordinate-concept coherence)
...
See link:docs/design/partitive-relation.md[] for the design rationale,
and `schemas/v3/examples/20-partitive-relation-closed.yaml` through
`29-partitive-relation-plurality-uncertain.yaml` for example payloads.

Binary `broader_partitive` edges and `PartitiveRelation` coexist;
no automatic consolidation is performed. The
`check-binary-has-part-redundancy` validator warns when binary `has_part`
edges duplicate PartitiveRelation assertions.
